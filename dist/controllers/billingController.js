"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.getBillingUsage = exports.verifyBillingCheckout = exports.createBillingCheckout = exports.getBillingPlans = void 0;
const types_1 = require("../types");
const flutterwaveService_1 = require("../services/flutterwaveService");
const quotaService_1 = require("../services/quotaService");
const parseCheckoutPlan = (rawPlan) => {
    if (!rawPlan)
        return null;
    const normalized = rawPlan.toLowerCase().trim();
    if (normalized === types_1.SubscriptionPlan.PRO)
        return types_1.SubscriptionPlan.PRO;
    if (normalized === types_1.SubscriptionPlan.ENTERPRISE)
        return types_1.SubscriptionPlan.ENTERPRISE;
    if (normalized === types_1.SubscriptionPlan.FREE)
        return types_1.SubscriptionPlan.FREE;
    return null;
};
const getBillingPlans = async (req, res, next) => {
    try {
        if (!req.user) {
            return res.status(401).json({ error: 'Not authenticated' });
        }
        const plans = (0, flutterwaveService_1.getBillingPlanCatalog)();
        return res.status(200).json({
            plans,
            current: req.user.subscription,
        });
    }
    catch (err) {
        next(err);
    }
};
exports.getBillingPlans = getBillingPlans;
const createBillingCheckout = async (req, res, next) => {
    try {
        if (!req.user) {
            return res.status(401).json({ error: 'Not authenticated' });
        }
        const selectedPlan = parseCheckoutPlan(req.body?.plan);
        if (!selectedPlan) {
            return res.status(400).json({ error: 'Invalid subscription plan selected' });
        }
        if (selectedPlan === types_1.SubscriptionPlan.FREE) {
            return res.status(400).json({ error: 'Free plan does not require checkout' });
        }
        const quantity = Number(req.body?.quantity || 1);
        const session = await (0, flutterwaveService_1.createFlutterwaveCheckoutSession)(req.user._id.toString(), selectedPlan, Number.isFinite(quantity) && quantity > 0 ? quantity : 1);
        return res.status(200).json({
            success: true,
            checkoutUrl: session.url,
            txRef: session.tx_ref || session.id,
            plan: selectedPlan,
        });
    }
    catch (err) {
        next(err);
    }
};
exports.createBillingCheckout = createBillingCheckout;
const verifyBillingCheckout = async (req, res, next) => {
    try {
        if (!req.user) {
            return res.status(401).json({ error: 'Not authenticated' });
        }
        const txRef = typeof req.body?.txRef === 'string' ? req.body.txRef : undefined;
        const transactionId = typeof req.body?.transactionId === 'string' ? req.body.transactionId : undefined;
        const plan = typeof req.body?.plan === 'string' ? req.body.plan : undefined;
        const verification = await (0, flutterwaveService_1.verifyFlutterwaveTransaction)(req.user._id.toString(), {
            txRef,
            transactionId,
            plan,
        });
        req.user.subscription.plan = verification.plan;
        return res.status(200).json({
            success: true,
            verification,
        });
    }
    catch (err) {
        next(err);
    }
};
exports.verifyBillingCheckout = verifyBillingCheckout;
const getBillingUsage = async (req, res, next) => {
    try {
        if (!req.user) {
            return res.status(401).json({ error: 'Not authenticated' });
        }
        const usage = await (0, quotaService_1.getCurrentUsageSnapshot)(req.user);
        return res.status(200).json({
            success: true,
            usage,
            subscription: req.user.subscription,
        });
    }
    catch (err) {
        next(err);
    }
};
exports.getBillingUsage = getBillingUsage;
