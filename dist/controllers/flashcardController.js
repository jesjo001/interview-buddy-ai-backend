...
exports.deleteFlashcard = exports.reviewFlashcard = exports.updateFlashcard = exports.createCustomFlashcard = exports.getFlashcardsByPrepId = exports.getFlashcardsByTopicId = exports.getFlashcardsDueForReview = void 0;
...
const getFlashcardsDueForReview = async (req, res, next) => {
    try {
        if (!req.user)
        ...
        if (prepId) {
            ...
            const topicIds = await Topic_1.default.find({ interviewPrepId: prepId }).select('_id');
            query.topicId = { $in: topicIds.map(topic => topic._id) };
        }
        const flashcards = await Flashcard_1.default.find(query)
            .limit(Number(limit))
...
exports.getFlashcardsDueForReview = getFlashcardsDueForReview;
const getFlashcardsByTopicId = async (req, res, next) => {
    try {
        if (!req.user)
            return res.status(401).json({ message: 'Unauthorized' });
        ...
        const prep = await InterviewPrep_1.default.findOne({ _id: topic.interviewPrepId, userId });
        if (!prep) {
            return res.status(401).json({ message: 'Unauthorized to access this topic' });
        }
        const flashcards = await Flashcard_1.default.find({ topicId, userId }).sort({ createdAt: 1 });
        res.status(200).json(flashcards);
    }
    catch (err) {
        next(err);
...
exports.getFlashcardsByTopicId = getFlashcardsByTopicId;
const getFlashcardsByPrepId = async (req, res, next) => {
    try {
        if (!req.user)
            return res.status(401).json({ message: 'Unauthorized' });
        ...
        const topicIds = await Topic_1.default.find({ interviewPrepId: prepId }).select('_id');
        const flashcards = await Flashcard_1.default.find({ topicId: { $in: topicIds.map(topic => topic._id) } });
        res.status(200).json(flashcards);
    }
    catch (err) {
        next(err);
...
exports.getFlashcardsByPrepId = getFlashcardsByPrepId;
const createCustomFlashcard = async (req, res, next) => {
    try {
        if (!req.user)
            return res.status(401).json({ message: 'Unauthorized' });
        const { error, value } = validators_1.createFlashcardSchema.validate(req.body);
        if (error) {
        ...
        const newFlashcard = await Flashcard_1.default.create({
            topicId,
            userId,
            front,
            back,
...
exports.createCustomFlashcard = createCustomFlashcard;
const updateFlashcard = async (req, res, next) => {
    try {
        ...
        const { id } = req.params;
        const userId = req.user._id;
        const { error, value } = validators_1.updateFlashcardSchema.validate(req.body);
        if (error) {
            return res.status(400).json({ error: error.details[0].message });
        }
        const flashcard = await Flashcard_1.default.findOne({ _id: id, userId });
        if (!flashcard) {
...
const reviewFlashcard = async (req, res, next) => {
    try {
        ...
        const userId = req.user._id;
        const { error, value } = validators_1.reviewFlashcardSchema.validate(req.body);
        if (error) {
            return res.status(400).json({ error: error.details[0].message });
        }
        ...
        flashcard.reviewSchedule = (0, spacedRepetitionService_1.calculateNextReview)(rating, flashcard.reviewSchedule);
        ...
        const topic = await Topic_1.default.findById(flashcard.topicId);
        if (topic) {
            const prep = await InterviewPrep_1.default.findById(topic.interviewPrepId);
            if (prep) {
...
const deleteFlashcard = async (req, res, next) => {
    try {
        ...
        const { id } = req.params;
        const userId = req.user._id;
        const flashcard = await Flashcard_1.default.findOneAndDelete({ _id: id, userId });
        if (!flashcard) {
            return res.status(404).json({ message: 'Flashcard not found or not authorized' });
        }
        // Optionally update InterviewPrep.progress.totalFlashcards here
